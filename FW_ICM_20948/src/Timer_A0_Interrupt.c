...
void Timer_A0_Interrupt_Init(void(*task)(void), uint16_t period)
{
     ...
     * @details
     *
     * Under this timer configuration, the timer-based interrupt triggers
     *  at the time declared at CCR[0].
     *
     * With SMCLK as a clock (at 12 MHz) and a total division of:
     *  - ID * EX0 = 4 * 5 = 20, and
    ...
    TIMER_A0->CTL  |= 0x0004;   // TACLR
    TIMER_A0->CTL  |= 0x0010;   // MC
