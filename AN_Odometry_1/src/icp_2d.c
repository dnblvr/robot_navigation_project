    ...
    float  *out_t)   // 2x1 translation vector
{
    ...
    src_trans = (Point2D*)malloc(source_size * sizeof(Point2D));
    ...
    for (iter = 0; iter < max_iterations; iter++) {
        ...
        correspondences = (int*)malloc(source_size * sizeof(int));
        if (!correspondences) {
            free(src_trans);
            return;
        }
        ...
        tgt_corr = (Point2D*)malloc(source_size * sizeof(Point2D));
        if (!tgt_corr) {
        ...
        mean_error = 0.0f;
        for (i = 0; i < source_size; i++) {
            dx = src_trans[i].x - tgt_corr[i].x;
            dy = src_trans[i].y - tgt_corr[i].y;
