 ...
 *
 * @note This function assumes that the necessary pin configurations for I2C
 *      communication have been performed on the corresponding pins. The output
 *      from the pins will be observed using an oscilloscope.
 *       - P6.4 (SDA)
 *       - P6.5 (SCL)
 *
 * For more information regarding the Enhanced Universal Serial Communication
 *      Interface (eUSCI), refer to the MSP432Pxx Microcontrollers Technical
 *      Reference Manual
 *
 * @author Aaron Nanas
 *
...
void EUSCI_B1_I2C_Init()
{
    ...
    // Configure the EUSCI_B1 module to operate as a single master by clearing
    // the UCMM bit (Bit 13) in the UCBxCTLW0 register
    EUSCI_B1->CTLW0    &= ~0x2000;


    // Configure the EUSCI_B1 module to operate in master mode by setting the
    // UCMST bit (Bit 11) in the UCBxCTLW0 register
    ...
    // Enable synchronous mode for the EUSCI_B1 module by setting the UCSYNC
    // bit (Bit 8) in the UCBxCTLW0 register
    EUSCI_B1->CTLW0    |=  0x0100;


    // Select SMCLK as the clock source for the EUSCI_B1 module by writing a
    // value of 11b (0x3) in the UCSSELx field (Bits 7 to 6) in the UCBxCTLW0
    // register
    EUSCI_B1->CTLW0    |=  0x00C0;


    // Configure the EUSCI_B1 module to not transmit the ACK condition in slave
    // mode with enabled address mask by clearing the UCTXACK bit (Bit 5) in
    // the UCBxCTLW0 register
    EUSCI_B1->CTLW0    &= ~0x0020;


    // Configure the EUSCI_B1 module to operate in I2C master receiver mode by
    // clearing the UCTR bit (Bit 4) in the UCBxCTLW0 register
    EUSCI_B1->CTLW0    &= ~0x0010;


    // Clear the UCTXNACK bit (Bit 3) in the UCBxCTLW0 register since it is
    // used only in slave receiver mode
    EUSCI_B1->CTLW0    &= ~0x0008;


    // Configure the EUSCI_B1 module not to generate the STOP condition in
    // master mode by clearing the UCTXSTP bit (Bit 2) in the UCBxCTLW0
    // register. Note that in master receiver mode, the STOP condition is
    // preceded by a NACK
    EUSCI_B1->CTLW0    &= ~0x0004;


    // Configure the EUSCI_B1 module to not generate the START condition in
    // master mode by clearing the UCTXSTT bit (Bit 1) in the UCBxCTLW0
    // register. Note that in master receiver mode, the START condition is
    // preceded by a NACK
    ...
    // Set the I2C clock prescaler value to 30 to divide the SMCLK clock
    // frequency from 12 MHz to 400 kHz
    //      - N = (Clock Frequency) / (SCL Frequency)
    //          = (12,000,000 / 400,000) = 30
    EUSCI_B1->BRW       =  30;


    // Configure the P6.4 (SDA) and P6.5 (SCL)  pins to use the primary module
    // function (I2C) by setting Bits 5 to 4 in the SEL0 register and clearing
    // Bits 5 to 4 in the SEL1 register
    P6->SEL0           |=  0x30;
    P6->SEL1           &= ~0x30;
        ...
        uint8_t     data)
{
    ...
    // Set the UCTR bit (Bit 4) in the UCBxCTLW0 register to configure the
    // EUSCI_B1 module in master transmitter mode. Then, clear the UCTXSTP bit
    // (Bit 2) to not generate the STOP condition. Lastly, set the UCTXSTT bit
    // (Bit 1) to generate the START condition
        ...
        uint32_t    packet_length)
{
    ...
    for (int i = 0; i < packet_length; i++)
    {
        ...
        // Store the 8-bit data in the Transmit Buffer by writing the data to
        // the UCBxTXBUF register
        ...
        uint8_t     slave_address)
{
    ...
    // Set the byte counter threshold to 1 by writing to the UCTBCNTx field in
    // the UCBxTBCNT register. This register can only be modified when the
    // UCSWRST bit (Bit 0) in the UCBxCTLW0 register is set
    ...
    // EUSCI_B1 module in master receiver mode. Then, set the UCTXSTP bit (Bit
    // 2) to generate the STOP condition. Lastly, set the UCTXSTT bit (Bit 1)
