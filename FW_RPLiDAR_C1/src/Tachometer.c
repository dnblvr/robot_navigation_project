 ...
 *  - P 5.0 (Right Encoder B)
 *  - P10.5 (Left Encoder A)
 *  - P 5.2 (Left Encoder B)
 *
 * @author Aaron Nanas
 */
...
// Global variable used to store the time of the previous & current rising edge for the right wheel
static uint16_t Tachometer_Previous_Right_Time,
                Tachometer_Current_Right_Time;


// Global variable used to store the time of the previous & current rising edge for the left wheel
static uint16_t Tachometer_Previous_Left_Time,
                Tachometer_Current_Left_Time;
...
{
    // Configure the following pins as input GPIO pins:
    //      - P5.0 (Right Encoder B) and
    //      - P5.2 (Left Encoder B)
    // by:
    //      - clearing b2,0 of SEL0 and SEL1 registers for P5 and
    //      - setting  b2,0 of the DIR register for P5
    P5->SEL0   &=  ~0x05;
    P5->SEL1   &=  ~0x05;
    P5->DIR    &=  ~0x05;


    // Initialize Timer A3 Capture (from Timer_A3_Capture.c)
