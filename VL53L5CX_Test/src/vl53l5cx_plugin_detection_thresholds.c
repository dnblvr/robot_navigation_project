		...
		uint8_t     *p_enabled)
{
	...
	status |= vl53l5cx_dci_read_data(
	        p_dev,
	        (uint8_t*)p_dev->temp_buffer,
			VL53L5CX_DCI_DET_THRESH_GLOBAL_CONFIG,
			8
	);
		...
		uint8_t     enabled)
{
	...
	status |= vl53l5cx_dci_replace_data(
	        p_dev,
	        p_dev->temp_buffer,
			VL53L5CX_DCI_DET_THRESH_CONFIG,
			20,
			(uint8_t*)&tmp,
			1,
			0x11
...
{
	uint8_t i, status = VL53L5CX_STATUS_OK;
	uint8_t grp_valid_target_cfg[] = {0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05};
