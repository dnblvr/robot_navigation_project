    ...
    const ICPResult     *result)
{
    ...
    for (int i = 0; i < transformed.num_points; i++) {
        float min_dist = FLT_MAX;
        
        for (int j = 0; j < scan2->num_points; j++) {
            float dx = transformed.points[i].x - scan2->points[j].x;
            float dy = transformed.points[i].y - scan2->points[j].y;
    ...
    float mean_error = total_error / matches;
    float match_ratio = (float)matches / transformed.num_points;
    
    // Confidence based on error and match ratio
    float error_confidence = expf(-mean_error * 2.0f);
    float confidence = error_confidence * match_ratio;
    ...
    float           confidence)
{
    ...
    float info_xy = confidence;
    float info_theta = confidence * 0.5f;
    
    // Compute error
    Pose *p1 = &optimizer->pose_pool[pose1_id];
    Pose *p2 = &optimizer->pose_pool[pose2_id];
    
    float z_ij[3] = {dx, dy, dtheta};
    float error[3];
    ...
    float omega[3] = {info_xy, info_xy, info_theta};
    ...
    int             current_pose_id)
{
    ...
    Pose *current_pose = &optimizer->pose_pool[current_pose_id];
    PointCloud *current_scan = &optimizer->scan_pool[current_pose_id];
    
    // Check spatially close poses
    for (int candidate = 0; candidate < current_pose_id - MIN_TEMPORAL_GAP; candidate++) {
        Pose *candidate_pose = &optimizer->pose_pool[candidate];
        
        float dist = pose_distance(current_pose, candidate_pose);
        
        if (dist < LOOP_DISTANCE_THRESHOLD) {
            // Attempt ICP alignment
            PointCloud *candidate_scan = &optimizer->scan_pool[candidate];
            ...
            float confidence = slam_compute_icp_confidence(
    ...
    int             max_iterations)
{
    ...
    int state_size = optimizer->current_pose_count * 3;
    
    // Convert sparse matrix to dense for Cholesky (for embedded system)
    float *H_dense = (float*)malloc(state_size * state_size * sizeof(float));
    if (!H_dense) return;
    ...
    for (int i = 0; i < optimizer->H.nnz; i++) {
    ...
    for (int iter = 0; iter < max_iterations; iter++) {
        ...
        float *dx = (float*)malloc(state_size * sizeof(float));
        ...
        int8_t result = cholesky_decompose(H_dense, L, state_size);
        ...
        // Create -b
        float *neg_b = (float*)malloc(state_size * sizeof(float));
        if (!neg_b) {
            free(L);
            free(y);
            free(dx);
        ...
        for (int p = 0; p < optimizer->current_pose_count; p++) {
            int idx = p * 3;
