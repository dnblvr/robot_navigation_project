...
#define STATEMENT (j == 2 || j == 4)
    ...
    // calculates the SE(2) transformation matrix for the current `action`
    float c, s;
    if (pose.theta < 1e-3f) {
        c = 1.f;  s = 0.f;
 ...
 */
    ...
    for (i = 0; i < output->num_pts; i++) {
        ...
        /**
         * @todo the transformation here has to negate the y-component to correct the flipped output on `pointcloud_visualizer`. find a better way to handle this before the transformation
         */
